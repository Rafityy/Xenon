...
# Main function to generate serialized hex strings for GET and POST C2 transformations
def generate_raw_c2_transform_definitions(data):
    ...
    def build_definitions(req_type, config, section):
        ...
        is_server = section == 'server'
        if is_server:
            transforms = transforms[::-1]  # Reverse order for server
        if transforms:
            definitions += extract_transforms(transforms, is_server=is_server)
        
        # DO NOT CHANGE ORDER
        # Insert payload location action LAST as the client will unpack buffer in order
...
# Main function to load JSON file from command line argument
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 test_settings_malleable.py <json_file_path>")
        sys.exit(1)
    ...
    try:
        ...
        get_client, post_client, get_server, post_server = generate_raw_c2_transform_definitions(data)
        
        print(f'#define S_C2_GET_CLIENT "{get_client}"')
        print(f'#define S_C2_POST_CLIENT "{post_client}"')
        print(f'#define S_C2_GET_SERVER "{get_server}"')
        print(f'#define S_C2_POST_SERVER "{post_server}"')
